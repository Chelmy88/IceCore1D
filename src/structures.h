...
enum SAVE_TYPE_ENUM {MATRIX, VECTOR};
enum SCHEME_ENUM {CN, EXPL};
enum THERMAL_ENUM {TH_CP, TH_GO};
enum FIRN_ENUM {FI_SC, FI_CP, FI_FI};
enum RHO_TYPE_ENUM {RHO_FIRN, RHO_CONST};
enum VERTICAL_ENUM {VE_FI, VE_PA};
enum INTERNAL_ENERGY_ENUM {ON, OFF};
enum MELTING_ENUM {FREE_MELT, FREEZING_NO_ICE, FREEZING};
...
typedef struct _model_parameters {
	...
	char* DIR_PATH1; //"output"//Directory to store data file into
	...
	enum VERTICAL_ENUM VERTICAL1;// "FI" // Set the flux shape function to FI or PA
